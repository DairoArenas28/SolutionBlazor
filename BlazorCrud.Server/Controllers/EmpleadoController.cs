...
namespace BlazorCrud.Server.Controllers
{
    ...
    public class EmpleadoController : ControllerBase
    {
        ...
        public async Task<IActionResult> Lista()
        {
            ...
            try
            {
                // Obtiene la lista de empleados y sus departamentos de la base de datos
                var empleados = await _dbContext.Empleados
                    .Include(e => e.IdDepartamentoNavigation)
                    .ToListAsync();

                // Mapea cada empleado a su correspondiente DTO
                listaEmpleadoDTO = empleados.Select(item => new EmpleadoDTO
                {
                ...
                // Configura la respuesta como correcta y asigna la lista de empleados
                responseApi.EsCorrecto = true;
                responseApi.Valor = listaEmpleadoDTO;
            }
            catch (Exception ex)
            {
                // En caso de excepción, configura la respuesta como incorrecta y asigna el mensaje de error
                responseApi.EsCorrecto = false;
                responseApi.Mensaje = ex.Message;
            }

            // Devuelve la respuesta API
